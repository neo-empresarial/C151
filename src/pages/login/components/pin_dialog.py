...
class PinDialog:
    def __init__(self, on_success):
        ...
        with self.dialog, ui.card().classes('w11-card w-[420px] items-center p-10 bg-opacity-90'):
            ui.label(lm.t('access_via_pin')).classes('text-3xl font-light mb-10 tracking-tight text-center w-full')
            ...
            with ui.grid(columns=3).classes('gap-6 w-full px-4'):
                ...
                ui.button(lm.t('enter'), on_click=self.verify_pin) \
    ...
    def verify_pin(self):
        ...
        if user:
            ...
            ui.notify(lm.t('invalid_pin'), type='negative')
            self.pin_input.value = ''

def render_trigger_button(on_click):
    with ui.row().classes('w-full justify-center absolute bottom-10 z-20'):
         ui.button(lm.t('enter_with_pin'), on_click=on_click).classes('w11-btn bg-white px-12 py-4 text-lg backdrop-blur-md shadow-2xl')
